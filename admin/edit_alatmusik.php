...
// Ambil data kuliner
$query = mysqli_query($conn, "SELECT * FROM alat_musik WHERE id = $id");
...
// ✅ TARO CEK NAMA DI SINI
    $cekNama = mysqli_query($conn, "SELECT * FROM alat_musik WHERE nama = '$nama' AND id != $id");
    if (mysqli_num_rows($cekNama) > 0) {
        echo "<script>
                alert('Nama \"$nama\" sudah ada, silakan gunakan nama lain!');
                window.location = 'edit_alatmusik.php?id=$id';
              </script>";
        exit;
    }
    ...
    // Update database
    $update = mysqli_query($conn, "UPDATE alat_musik SET 
        nama = '$nama',
        deskripsi = '$deskripsi',
        gambar = '$gambar'
        WHERE id = $id");
    ...
    if ($update) {
        echo "<script>
                ...
                window.location.href = 'alatmusik.php';
              </script>";
        exit;
    } else {
...
<body>
    <div class="header">Edit Alat musik</div>

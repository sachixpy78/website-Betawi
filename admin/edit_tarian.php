...
// Ambil data tarian
$query = mysqli_query($conn, "SELECT * FROM tarian WHERE id = $id");
...
// ✅ TARO CEK NAMA DI SINI
    $cekNama = mysqli_query($conn, "SELECT * FROM tarian WHERE nama = '$nama' AND id != $id");
    if (mysqli_num_rows($cekNama) > 0) {
        echo "<script>
                alert('Nama \"$nama\" sudah ada, silakan gunakan nama lain!');
                window.location = 'edit_tarian.php?id=$id';
              </script>";
        exit;
    }
    // Update database
    $update = mysqli_query($conn, "UPDATE tarian SET 
        nama = '$nama',
        deskripsi = '$deskripsi',
        link = '$link'
        WHERE id = $id");
    ...
    if ($update) {
        echo "<script>
                ...
                window.location.href = 'tarian.php';
              </script>";
        exit;
    } else {
...
<body>
    <div class="header">Edit Tarian</div>

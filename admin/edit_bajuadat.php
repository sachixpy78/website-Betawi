...
// Cek ID dari URL
if (!isset($_GET['id'])) {
    header("Location: bajuadat.php");
    exit;
}
$id = intval($_GET['id']);

// Ambil data kuliner
$query = mysqli_query($conn, "SELECT * FROM baju_adat WHERE id = $id");
...
// ✅ TARO CEK NAMA DI SINI
    $cekNama = mysqli_query($conn, "SELECT * FROM baju_adat WHERE nama = '$nama' AND id != $id");
    if (mysqli_num_rows($cekNama) > 0) {
        echo "<script>
                alert('Nama \"$nama\" sudah ada, silakan gunakan nama lain!');
                window.location = 'edit_bajuadat.php?id=$id';
              </script>";
        exit;
    }
    ...
    // Update database
    $update = mysqli_query($conn, "UPDATE baju_adat SET 
        nama = '$nama',
        deskripsi = '$deskripsi',
        gambar = '$gambar'
        WHERE id = $id");
    ...
    if ($update) {
        echo "<script>
                ...
                window.location.href = 'bajuadat.php';
              </script>";
        exit;
    } else {
...
<body>
    <div class="header">Edit Baju Adat</div>
    ...
    <div class="container">
        <form method="POST" enctype="multipart/form-data">
            ...
            <label>Nama Baju Adat:</label>
            <input type="text" name="nama" value="<?= htmlspecialchars($data['nama']); ?>" required>

            <label>Deskripsi Baju Adat:</label>
